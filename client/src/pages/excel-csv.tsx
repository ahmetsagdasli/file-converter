...
import { ArrowLeft, RefreshCw, ArrowRight, Loader2, CheckCircle, X } from "lucide-react";
import type { UploadedFile, ProcessedFile, ProcessingStatus } from "@/types";

const steps = [1, 2, 3];

export default function ExcelCsv() {
  ...
  const convertMutation = useMutation({
    mutationFn: (data: any) => {
      if (conversionType === 'excel-to-csv') {
        return api.convertExcelToCsv(data);
      } else {
        return api.convertCsvToExcel(data);
      }
    },
    onMutate: () => {
      setProcessingStatus({
  ...
  const handleFilesUploaded = (uploadedFiles: UploadedFile[]) => {
    ...
    if (uploadedFiles.length > 0 && !filename) {
      ...
      const extension = conversionType === 'excel-to-csv' ? '.csv' : '.xlsx';
  ...
  const acceptedTypes = conversionType === 'excel-to-csv' 
    ? '.xls,.xlsx' 
    : '.csv';
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Convert between Excel spreadsheets and CSV files
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-4 mt-2">
                  <Button
                    variant={conversionType === 'excel-to-csv' ? 'default' : 'outline'}
                    onClick={() => {
                      setConversionType('excel-to-csv');
                      setFiles([]);
                      setFilename("");
                    }}
                    data-testid="excel-to-csv-btn"
                  >
                    Excel → CSV
                  </Button>
                  <Button
                    variant={conversionType === 'csv-to-excel' ? 'default' : 'outline'}
                    onClick={() => {
                      setConversionType('csv-to-excel');
                      setFiles([]);
                      setFilename("");
                    }}
                    data-testid="csv-to-excel-btn"
                  >
                    CSV → Excel
                  </Button>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={`converted_file${conversionType === 'excel-to-csv' ? '.csv' : '.xlsx'}`}
                    className="mt-1"
