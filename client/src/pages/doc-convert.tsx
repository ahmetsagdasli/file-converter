...
import { ArrowLeft, FileEdit, ArrowRight, Loader2, CheckCircle, X } from "lucide-react";
...
export default function DocConvert() {
  ...
  const [filename, setFilename] = useState("converted_document.pdf");
  ...
  const convertMutation = useMutation({
    mutationFn: api.convertDocToPdf,
    onMutate: () => {
      setProcessingStatus({
        status: 'processing',
        progress: 0,
  ...
  const handleFilesUploaded = (uploadedFiles: UploadedFile[]) => {
    ...
    if (uploadedFiles.length > 0 && !filename) {
      ...
      setFilename(`${originalName}_converted.pdf`);
    }
  };
  ...
  const handleStartNew = () => {
    setFiles([]);
    setFilename("converted_document.pdf");
    setResult(null);
    setCurrentStep(1);
              ...
              <div className="text-center py-8">
                {processingStatus.status === 'processing' ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-muted-foreground">
                      Your document has been successfully converted to PDF
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
